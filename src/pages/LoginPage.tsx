    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="bg-card rounded-2xl shadow-card p-8 space-y-6 border border-border">
          <h2 className="text-xl font-display font-semibold text-center">{getTitle()}</h2>

          <form onSubmit={handleSubmit} className="space-y-4">
            {isSignup && !isForgotPassword && (
              <div className="space-y-2">
                <Label htmlFor="name" className="font-body text-sm font-medium">Name</Label>
                <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" className="font-body" />
              </div>
            )}
            <div className="space-y-2">
              <Label htmlFor="email" className="font-body text-sm font-medium">Email</Label>
              <Input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="font-body" />
            </div>
            {!isForgotPassword && (
              <div className="space-y-2">
            ...
            <Button type="submit" disabled={submitting} className="w-full bg-secondary text-secondary-foreground hover:bg-secondary/90 font-body font-medium shadow-gold transition-all">
              {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : isForgotPassword ? "Send Reset Link" : isSignup ? "Create Account" : "Sign In"}
            </Button>
          </form>
          ...
          {!isForgotPassword && (
            <>
              ...
              <Button variant="outline" onClick={handleGoogleSignIn} className="w-full font-body gap-2">
          ...
          {!isSignup && !isForgotPassword && (
            <p className="text-center">
              <button onClick={() => setIsForgotPassword(true)} className="text-sm text-secondary font-body font-medium hover:underline">
          ...
          <p className="text-center text-sm text-muted-foreground font-body">
            {isForgotPassword ? (
              <button onClick={() => setIsForgotPassword(false)} className="text-secondary font-medium hover:underline">Back to sign in</button>
            ) : (
              <>
